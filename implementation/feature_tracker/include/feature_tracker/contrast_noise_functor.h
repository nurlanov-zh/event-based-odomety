...
namespace tracker
{
struct contrastNoiseFunctor
{
	contrastNoiseFunctor(const std::list<common::EventSample>& events,
						 const cv::Rect2i patchRect,
						 const double compensateScale)
		: events_(events),
	...
	{
		const T* motion = parameters[0];
		Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic,
									   Eigen::RowMajor>>
			noiseMat(parameters[1], patchRect_.height, patchRect_.width);

		Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic> compensatedEventImage_;
		compensatedEventImage_.setZero(3 * patchRect_.height,
									   3 * patchRect_.width);

		compensateEvents(motion, compensatedEventImage_, noiseMat);
	...
	template <typename T>
	void compensateEvents(
		const T* motion,
		Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic>& compensatedEventImage,
		Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic,
									   Eigen::RowMajor>>& noiseMat) const
	{
				 ...
				 i <= gaussianCompensateKernelSize_; i++)
			{
					 ...
					 j <= gaussianCompensateKernelSize_; j++)
				{
						...
						pointY >= 0 && pointY < 3 * patchRect_.height)
					{
						...
						const T noise =
							noiseMat(event.value.point.y - patchRect_.y,
									 event.value.point.x - patchRect_.x);

						compensatedEventImage(pointY, pointX) +=
							ceres::exp(noise) / (ceres::exp(noise) + T(1)) *
							value;
					}
				}
			}
