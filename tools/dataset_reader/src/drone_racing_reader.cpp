...
CameraModelParams<double> DroneRacingReader::getCalibrationLine(
...
EventSample DroneRacingReader::getEventSample(std::string& line) const
{
	size_t pos = line.find(' ');
	const auto duration =
...
tracker::Patch DroneRacingReader::getTrajectoryLine(std::string& line) const
{
	size_t pos = line.find(' ');
...
std::optional<EventSequence> DroneRacingReader::getEvents()
{
	...
	const auto events = readFile<EventSequence, EventSample>(
		filePath,
		std::bind(&DroneRacingReader::getEventSample, this,
				  std::placeholders::_1),
...
ImageSequence DroneRacingReader::getImages() const
{
	...
	const auto images = readFile<ImageSequence, ImageSample>(
		filePath,
		std::bind(&DroneRacingReader::getImageSample, this,
				  std::placeholders::_1),
...
GroundTruth DroneRacingReader::getGroundTruth() const
{
	...
	const auto groundTruth = readFile<GroundTruth, GroundTruthSample>(
		filePath,
		std::bind(&DroneRacingReader::getGroundTruthSample, this,
				  std::placeholders::_1),
		0, EVENT_LENGTH, numOfStrings);
	const auto end = std::chrono::high_resolution_clock::now();
...
CameraModelParams<double> DroneRacingReader::getCalibration() const
{
	...
	const auto calibration = readFile<std::vector<CameraModelParams<double>>,
		...
		filePath,
		std::bind(&DroneRacingReader::getCalibrationLine, this,
				  std::placeholders::_1),
		0, EVENT_LENGTH, numOfStrings);
	const auto end = std::chrono::high_resolution_clock::now();
...
tracker::Patches DroneRacingReader::getTrajectory() const
{
